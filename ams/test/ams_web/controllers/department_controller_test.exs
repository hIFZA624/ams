defmodule AmsWeb.DepartmentControllerTest do
  use AmsWeb.ConnCase
  ...
  def fixture(:department) do
    {:ok, department} = Payroll.create_department(@create_attrs)
    department
  end
  ...
  describe "new department" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.department_path(conn, :new))
  ...
  describe "create department" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.department_path(conn, :show, id)
  ...
  describe "edit department" do
    ...
    test "renders form for editing chosen department", %{conn: conn, department: department} do
      conn = get(conn, Routes.department_path(conn, :edit, department))
  ...
  describe "update department" do
    ...
    test "redirects when data is valid", %{conn: conn, department: department} do
      ...
      assert redirected_to(conn) == Routes.department_path(conn, :show, department)

      conn = get(conn, Routes.department_path(conn, :show, department))
  ...
  describe "delete department" do
    ...
    test "deletes chosen department", %{conn: conn, department: department} do
      ...
      assert redirected_to(conn) == Routes.department_path(conn, :index)
